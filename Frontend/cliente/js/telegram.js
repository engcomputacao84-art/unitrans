(function () {
  ...
  var elToast = document.getElementById('telegramToast');

  function toast(mensagem, tipo) {
    if (!elToast) return;
    elToast.textContent = mensagem;
    elToast.className = 'telegram-toast show' + (tipo === 'error' ? ' error' : '');
  ...
  function carregarStatus() {
    ...
    fetch(API_BASE + '/telegram/status/' + encodeURIComponent(clienteId))
      .then(function (res) {
  ...
  function gerarCodigo() {
    ...
    fetch(API_BASE + '/telegram/codigo', {
      ...
      .then(function (dados) {
        ...
        toast('Código gerado! Envie a mensagem para o bot no Telegram.', 'success');
      })
      .catch(function (err) {
        toast(err.message || 'Não foi possível gerar o código.', 'error');
      })
      .finally(function () {
  ...
  function desvincular() {
    if (!window.confirm('Tem certeza que deseja desvincular sua conta do Telegram? Você deixará de receber avisos por lá.')) {
      return;
    }
    ...
    fetch(API_BASE + '/telegram/desvincular', {
      ...
      .catch(function (err) {
        toast(err.message || 'Não foi possível desvincular.', 'error');
      });
  }
